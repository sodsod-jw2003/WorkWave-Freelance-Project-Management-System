...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validate OTP
    $stmt = $mysqli->prepare("SELECT id FROM v_user_credentials WHERE id = ? AND activation_token = ?");
    $stmt->bind_param("is", $_SESSION['temp_user_id'], $entered_otp);
    $stmt->execute();
    ...
    if ($user) {
        ...
        // Fetch user details
        $stmt = $mysqli->prepare("SELECT * FROM v_user_credentials WHERE id = ?");
        $stmt->bind_param("i", $_SESSION['temp_user_id']);
        $stmt->execute();
        $result = $stmt->get_result();
        $user = $result->fetch_assoc();
    ...
    <section class="container-fluid poppins vh-100 p-5 d-flex justify-content-center align-items-center position-relative">
        ...
        <div class="card p-5 rounded-5 shadow w-50 position-relative z-index-2">
            ...
            <div class="text-center mb-4">
                ...
                <h6 class="fs-6 text-dark">Enter OTP Code to Verify Your Account</h6>
            </div>
